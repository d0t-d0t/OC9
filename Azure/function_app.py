...
@app.route(route="Recommend")#, methods=["POST"], auth_level=func.AuthLevel.FUNCTION
def recommend_articles(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        if type(article_service)!=type(None):
            recommendations = article_service.get_recommendations(user_id, n_recommendations)
        else:
